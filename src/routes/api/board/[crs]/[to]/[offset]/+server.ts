...
export const GET: RequestHandler = async ({ params }) => {
	...
	if (shouldUseRailData) {
		const time = dayjs().tz('Europe/London').add(parseInt(offset || '0'), 'minute').format('YYYYMMDDTHHmmss');
		let urlObj = new URL(`https://api1.raildata.org.uk/1010-live-departure-board---staff-version1_0/LDBSVWS/api/20220120/GetDepartureBoardByCRS/${crs}/${time}`);
