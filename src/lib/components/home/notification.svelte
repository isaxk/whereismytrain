...
{#if service}
	<a
		...
		class={[
			'flex h-16 w-[320px] flex-col justify-center gap-0.5 border-border text-left text-base'
		]}
	>
				...
				{#if savedItem?.service?.callingPoints.find((point) => point.crs === data.focus_crs)}
					<div class="text-xs/3 font-light text-muted-foreground">
						<span class="font-medium">
							{savedItem?.service?.callingPoints.find((point) => point.crs === data.focus_crs)
								?.name}
						</span> to
					</div>
				{/if}
...
{:else}
	{title}
	<div class="text-xs">Could not find service</div>
{/if}
